...
function cachedQuery<T>(
  ...
  if (hit != null && Date.now() - hit.ts < hit.ttl) {
...
export const conversationApi = {
  ...
  chat: mut(client.conversation.chat, (i) => [
    i.conversationId != null && i.conversationId !== ""
      ? `conversation.getMessages:${i.conversationId}`
      : "conversation.getMessages:",
    `conversation.countUserMessages:`,
  ]),
  dislikeMessage: mut(client.conversation.dislikeMessage, () => []),
};
...
export const adminApi = {
  ...
  dislikedMessages: q(
    "admin.dislikedMessages",
    client.admin.dislikedMessages,
    30_000,
  ),
};
